...
def visualize_anchors(imgsz, anchors, logdir, name):
    # draw and save anchors
    img = np.zeros((imgsz, imgsz, 3), np.uint8)
    anchors = anchors[np.argsort(anchors.prod(1))]

    def _get_random_color(max_value=255):
    ...
    for a in anchors:
        ...
        stride += 10
    cv2.imwrite(os.path.join(logdir, '{}_anchors.jpg'.format(name)), img)
    np.save(os.path.join(logdir, '{}_anchors.npy'.format(name)), anchors)


def check_anchors(dataset, model, logdir, name, thr=4.0, imgsz=640):
    ...
    if bpr < 0.98:  # threshold to recompute
        ...
        visualize_anchors(imgsz, new_anchors, logdir, name)
    ...
    for i, o in enumerate(output):
        if o is not None:
            for pred in o:
                ...
                # y = box[1] / height + h / 2
                # conf = pred[4]
                w = ((box[2] - box[0]) / width).item()
                h = ((box[3] - box[1]) / height).item()
                x = (box[0] / width + w / 2).item()
                y = (box[1] / height + h / 2).item()
                conf = pred[4].item()
                cls = int(pred[5])
