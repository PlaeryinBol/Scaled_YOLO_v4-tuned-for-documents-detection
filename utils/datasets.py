...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      local_rank=-1, world_size=1, shuffle=False):
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=shuffle) if local_rank != -1 else None
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        hyp = self.hyp
        mosaic = self.mosaic and hyp['mosaic'] == 1
        if mosaic:
            # Load mosaic
        ...
        if self.augment:
            ...
            Colorjitter = albumentations.augmentations.transforms.ColorJitter(brightness=hyp['brightness'],
                                                                              contrast=hyp['contrast'],
                                                                              saturation=hyp['saturation'],
                                                                              hue=hyp['hue'],
                                                                              always_apply=True)
            img = Colorjitter(image=img)["image"]
        ...
        if self.augment:
            ...
            if random.random() < hyp['gaussnoise']:
                Gaussnoise = albumentations.augmentations.transforms.GaussNoise(always_apply=True)
                img = Gaussnoise(image=img)["image"]

            # invert
            if random.random() < hyp['invert']:
                Invert = albumentations.augmentations.transforms.InvertImg(p=1)
                img = Invert(image=img)["image"]

            # randomly shift values for each channel
            if random.random() < hyp['rgbshift']:
                RGBShift = albumentations.augmentations.transforms.RGBShift(r_shift_limit=255 * hyp['rgbshift'],
                                                                            g_shift_limit=255 * hyp['rgbshift'],
                                                                            b_shift_limit=255 * hyp['rgbshift'],
                                                                            always_apply=True)
                img = RGBShift(image=img)["image"]
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=xy.T, wh_thr=2, ar_thr=1000, area_thr=0.2)
        targets = targets[i]
        targets[:, 1:5] = xy[i]
